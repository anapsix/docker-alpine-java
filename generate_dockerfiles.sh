...
for version in ${JAVA_VERSIONS[@]}; do
  ...
  for JVM_FLAVOR in ${JVM_FLAVORS[@]}; do
    ...
    elif [ "${JVM_MAJOR}" -eq "7" ]; then
      gen_dockerfile $JVM_FLAVOR
    fi

  done

  if [ "${JVM_MAJOR}" -eq "9" ]; then
    JAVA_JCE="standard"
    gen_dockerfile server-jre9
    gen_dockerfile jdk9
  fi
done
